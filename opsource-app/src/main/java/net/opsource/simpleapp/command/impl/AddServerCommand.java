...
public class AddServerCommand extends AbstractCommand implements Command {
	...
	public CommandResponse execute(final CommandRequest commandRequest) {
		...
		try {
			serverMonitoringService.save(server);
			final String statusMessage = "Server with ID: " + server.getId() + ", Name: " + server.getName() + " saved successfully.";
